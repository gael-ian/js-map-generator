 ...
 */
map.draw.bidimensional.base = function(canvas) {
  map.draw.base.call(this, canvas);
};
map.utils.inherits(map.draw.bidimensional.base, map.draw.base);

map.draw.bidimensional.base.prototype.reset = function(canvas) {
  this.context = this.canvas.getContext('2d');
  
  this.clear();
};
      
map.draw.bidimensional.base.prototype.clear = function(color) {
  this.context.fillStyle = (color || "white");
  this.context.fillRect(0, 0, this.width, this.height);
};

map.draw.bidimensional.base.prototype.fillRegion = function(center, color) {
  var moved = false;
  
  center.corners.clockwise(function(co, i) {
    if (!moved) {
      this.context.beginPath();
      this.context.moveTo(this.x(co.point), this.y(co.point));
    } else {
      this.context.lineTo(this.x(co.point), this.y(co.point));
    }
    moved = true;
  }, this);
...
map.draw.bidimensional.base.prototype.defaultSteps = function() {
    ...
    callback: function(context, drawer) {
      this.edges.each(function(e) {
        ...
        e.corners.each(function(c) {
          if (!moved) {
            context.moveTo(drawer.x(c.point), drawer.y(c.point));
          } else {
            context.lineTo(drawer.x(c.point), drawer.y(c.point));
          }
          moved = true;
        }, this);
    ...
    callback: function(context, drawer) {
      this.centers.asQueue(function(c, queue, queued) {
        c.neighbors.each(function(n) {
          context.beginPath();
          context.moveTo(drawer.x(c.point), drawer.y(c.point));
          context.lineTo(drawer.x(n.point), drawer.y(n.point));
  ...
  steps.push({
    ...
    callback: function(context, drawer) {
      this.centers.each(function(c) {
        context.beginPath();
        context.arc(drawer.x(c.point), drawer.y(c.point), 1, 0, 2 * Math.PI, true);
